...
export async function triggerDiscoverFromUI(queries: DiscoverQuery[], dealsOnly = false, pages = 1): Promise<DiscoverAck> {
  let tabs: chrome.tabs.Tab[] = []
  try {
    tabs = await chrome.tabs.query({ url: LIST_TAB_MATCH })
  } catch {
  ...
  for (const t of tabs) {
    if (t.id == null) continue
    try {
      const ack = await sendWithTimeout<DiscoverAck>(t.id, { type: 'DISCOVER', queries, dealsOnly, pages }, 180_000)
      if (ack) return ack
    } catch {
      /* no content script here — try the next */
    }
