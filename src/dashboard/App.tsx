...
// Discover query-builder format options: label for the <select> + a matcher run
// against each result tile's format string. `any` has no matcher (no filtering).
const FORMAT_OPTIONS: { value: string; label: string; match?: (f: string) => boolean }[] = [
  { value: 'any', label: 'Any format' },
  { value: 'hardcover', label: 'Hardcover', match: (f) => /hardcover|hardback/i.test(f) },
  { value: 'paperback', label: 'Paperback', match: (f) => /paperback/i.test(f) && !/mass market/i.test(f) },
  { value: 'mass', label: 'Mass market', match: (f) => /mass market/i.test(f) },
  { value: 'library', label: 'Library binding', match: (f) => /library/i.test(f) },
]
const formatMatcher = (v: string) => FORMAT_OPTIONS.find((o) => o.value === v)?.match
...
export function App() {
  ...
  const [qDepth, setQDepth] = useState(() => { try { return Number(localStorage.getItem('tbw-q-depth')) || 100 } catch { return 100 } })
  ...
  useEffect(() => { try { localStorage.setItem('tbw-q-term', qTerm); localStorage.setItem('tbw-q-format', qFormat); localStorage.setItem('tbw-q-max', qMax); localStorage.setItem('tbw-q-mode', qMode); localStorage.setItem('tbw-q-depth', String(qDepth)) } catch { /* ignore */ } }, [qTerm, qFormat, qMax, qMode, qDepth])
  ...
  const runDiscover = async (catsOverride?: boolean, dealsOverride?: boolean) => {
    ...
    const queries: DiscoverQuery[] = [
      ...top(taste.author, 20).map((a): DiscoverQuery => ({ kind: 'author', term: a, label: a })),
      ...top(taste.publisher, 12, 2).map((p): DiscoverQuery => ({ kind: 'publisher', term: p, label: p })),
      ...(cats ? top(taste.category, 6).map((c): DiscoverQuery => ({ kind: 'category', term: CATEGORY_QUERY[c] ?? c, label: c })) : []),
    ]
  ...
  // Manual query-builder search: one targeted term, fetched a few pages deep, then
  // filtered by format + max price + deals right here (all present on the result tiles).
  const runSearch = async () => {
    const term = qTerm.trim()
    if (!term) { setCandidates([]); setDiscoverStatus('Type a press, author, title, or keyword to search.'); return }
    const maxCents = qMax.trim() ? Math.round(parseFloat(qMax) * 100) : null
    const match = formatMatcher(qFormat)
    const fmtLabel = qFormat !== 'any' ? FORMAT_OPTIONS.find((o) => o.value === qFormat)?.label : null
    const bits = [fmtLabel, maxCents != null ? `≤ ${formatCents(maxCents)}` : null, dealMode ? 'deals' : null].filter(Boolean).join(' · ')
    const tail = bits ? ` · ${bits}` : ''
    setDiscovering(true); setCandidates([])
    setDiscoverStatus(`Searching ThriftBooks for “${term}”${tail}…`)
    const ack = await triggerDiscoverFromUI([{ kind: 'manual', term, label: term }], dealMode, 3)
    setDiscovering(false)
    if (!ack.ok || !ack.candidates) { setCandidates([]); setDiscoverStatus(ack.error ?? 'Could not search — open your ThriftBooks wishlist in a tab and try again.'); return }
    const onWishlist = new Set(items.map((i) => i.productId).filter(Boolean) as string[])
    const results = ack.candidates
      .filter((c) => !onWishlist.has(c.workId))
      .filter((c) => (dealMode ? c.isDeal : true))
      .filter((c) => maxCents == null || (c.priceCents != null && c.priceCents <= maxCents))
      .filter((c) => !match || (c.format ? match(c.format) : false))
      .sort((a, b) => (a.priceCents ?? Number.MAX_SAFE_INTEGER) - (b.priceCents ?? Number.MAX_SAFE_INTEGER))
  ...
  const runCollect = async (more = false) => {
    ...
    setDiscoverStatus(`Finding “${name}” via Open Library, then checking up to ${qDepth} titles on ThriftBooks${qDepth >= 60 ? ' — give it a few minutes' : ''}…`)
    const ack = await triggerCollectFromUI(kind, name, { offset, limit: qDepth, maxCents })
    setDiscovering(false)
    ...
    setCollectOffset(offset + qDepth)
...
function DiscoverPanel({ candidates, discovering, status, onAdd, addedIds, ceiling, onSubmit, onSuggest, onLoadMore, canLoadMore, qMode, onMode, qDepth, setQDepth, qTerm, setQTerm, qFormat, setQFormat, qMax, setQMax, includeCats, setIncludeCats, dealMode, setDealMode, taste }: {
  ...
  qDepth: number
  setQDepth: (v: number) => void
  qTerm: string
  setQTerm: (v: string) => void
  qFormat: string
  setQFormat: (v: string) => void
  qMax: string
  setQMax: (v: string) => void
  includeCats: boolean
  setIncludeCats: (v: boolean) => void
  dealMode: boolean
  ...
  // Result view — list rows or a cover-gallery grid; remembered between sessions.
  const [view, setView] = useState<'list' | 'gallery'>(() => { try { return (localStorage.getItem('tbw-discover-view') as 'list' | 'gallery') || 'list' } catch { return 'list' } })
  useEffect(() => { try { localStorage.setItem('tbw-discover-view', view) } catch { /* ignore */ } }, [view])
  return (
    <div>
      <div className="mb-4 border-b-4 border-accent pb-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[13px]">
          ...
          {collectMode && (
            <label className="flex items-center gap-1 text-muted" title="How many catalog titles to check on ThriftBooks per run — deeper is slower (~1 title/second)">
              Depth
              <select value={qDepth} onChange={(e) => setQDepth(Number(e.target.value))} aria-label="Titles checked per run" className="rounded border border-line px-2 py-1.5 text-ink">
        ...
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-[13px] text-muted">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="inline-flex rounded border border-line p-0.5" role="group" aria-label="Result view">
              <button onClick={() => setView('list')} aria-pressed={view === 'list'} className={`rounded px-2 py-0.5 ${view === 'list' ? 'bg-teal-700 text-white' : 'text-muted hover:text-ink'}`}>List</button>
              <button onClick={() => setView('gallery')} aria-pressed={view === 'gallery'} className={`rounded px-2 py-0.5 ${view === 'gallery' ? 'bg-teal-700 text-white' : 'text-muted hover:text-ink'}`}>Gallery</button>
            </div>
